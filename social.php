...
foreach($instAccounts as $instAccount){
	...
	$photos = insGetPosts($instAccount, 3);
	if(is_array($photos)){
		echo "SUCCESS: ";
...
/*
				if(($instAccount=="287930161")||($instAccount=="1539646")){
					if($instAccount=="287930161")$twitterAccount='1086010394';
					if($instAccount=="1539646")$twitterAccount='25881149';
// tweet about new instagram
					$tweetText=$captionText;
					if(strlen($tweetText)>80){
						$tweetText=substr($tweetText, 0, 77);
						$tweetText.='...';
					}
					$tweetText.=' http://instagram.andybarefoot.com/photo.php?postid=';
					$tweetText.=$newPost;				
					$newTweetResponse=twNewPost($twitterAccount,$tweetText);
					$newTweet=json_decode($newTweetResponse);
					$newTweetID = $newTweet->id_str;
					$newTweetText = addslashes($newTweet->text);
					$newTweetDate=date('Y-m-d H:i:s',strtotime($newTweet->created_at));
					if($newTweet){
						$sqlStmt="SELECT * FROM `posts` WHERE `post_network` =1 AND `post_local_post` = '$newTweetID' LIMIT 0 , 30";
						$insTweets=getData($sqlStmt);
						if(!$insTweets){
							$newTweetText=addSlashes($newTweetText);
							$sqlStmt="INSERT INTO `db73815_social`.`posts` (`post_id`, `post_network`, `post_local_user`, `post_local_post`, `post_date`, `post_display`) VALUES (NULL, '1', '$twitterAccount', '$newTweetID', '$newTweetDate', '0')";
							$newPost=insertDataReturnID($sqlStmt);
							$sqlStmt="INSERT INTO `db73815_social`.`tweets` (`tweet_id`, `tweet_user`, `tweet_date`, `tweet_text`, `tweet_isReply`, `tweet_isRetweet`) VALUES ('$newTweetID', '$twitterAccount', '$newTweetDate', '$newTweetText', '0', '0');";
