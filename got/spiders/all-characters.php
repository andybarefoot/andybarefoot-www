...
//	$allCharacters = curlGet('http://gameofthrones.wikia.com/api/v1/Articles/List/?limit=10&category=Character');
	$allCharacters = curlGet('http://gameofthrones.wikia.com/api/v1/Articles/List/?category=Peoples');
...
function getAppearances($start,$number){
	...
	foreach($charResults as $char){
		$charID = $char['charID'];
		print "<tr>";	
		print "<td><a href='".$char['charURL']."' target='_blank'>".$char['charName']."</a></td>";
		$charPageSrc =  curlGet($char['charURL']);
		$thisPageXPath = returnXPathObject($charPageSrc);	// Instantiating new XPath DOM object
//		$thisStatus = $thisPageXPath->query("//table[.='appearances']/tbody/tr/td/b/a");	// Querying for href attributes of pagination
		...
		print "<td>".$thisSeries->length.":".$thisAppearances->length.":".$realApp."</td>";
