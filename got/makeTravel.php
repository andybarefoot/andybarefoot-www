...
//$sqlStmt="SELECT * FROM `gotAppearances` LEFT JOIN `gotChars` ON `appChar` = `charID` LEFT JOIN `gotPlaces` ON `appPlace` = `placeID` ORDER BY `appEpisode` ASC, `combOrder` ASC";
$sqlStmt="SELECT * FROM `gotAppearances` LEFT JOIN `gotChars` ON `appChar` = `charID` LEFT JOIN `gotPlaces` ON `appPlace` = `placeID` WHERE `charImportant` =1 ORDER BY `appEpisode` ASC, `combOrder` ASC";
...
for($i=1;$i<=60;$i++){
	...
	foreach ($appResults as $appKey=>$app) {
		...
		if($app['appEpisode']==$i){
			$key=false;
			$needle = $app['charID'];
			$haystack = $appearances[$app['appEpisode']-1][$app['combOrder']];
